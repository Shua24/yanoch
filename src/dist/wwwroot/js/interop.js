...
(function() {
    ...
    document.addEventListener('input', function(e) {
        if (slashMenuEl) {
            ...
            var val = e.target.tagName === 'DIV' ? (e.target.textContent || '') : e.target.value;
        ...
        var val = target.tagName === 'DIV' ? (target.textContent || '') : target.value;
    ...
    async function selectSlashItem(item) {
        ...
        if (el) { if (el.tagName === 'DIV') el.textContent = ''; else el.value = ''; }
    ...
    document.addEventListener('focusin', function(e) {
        var el = e.target.closest('input.block-textarea, input.block-input, div.block-textarea');
        if (!el) return;
...
function focusBlockInput(blockId) {
    ...
    if (!el) return;
    var input = el.querySelector('input, textarea, div[contenteditable]');
    if (input) {
        if (input.tagName === 'DIV') { var r = document.createRange(); r.selectNodeContents(input); r.collapse(false); var s = window.getSelection(); s.removeAllRanges(); s.addRange(r); }
        input.focus();
    }
}
