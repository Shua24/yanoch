...
function checkWiki(editor) {
  ...
  if (lastOpen !== -1) {
    ...
    if (afterOpen.indexOf(']]') === -1) {
      ...
      if (!wikiActive) openWiki(editor, absPos + 2)

      if (query !== wikiQuery) { wikiQuery = query; wikiIdx = 0; fetchWikiSuggestions(query) }
      return
    }
  }
...
export function setupWikiAutocomplete(editor) {
  return { 
    checkWiki: () => checkWiki(editor), 
    closeWiki,
    getState: () => ({ wikiActive, wikiMenuEl, wikiQuery, wikiItems, wikiIdx }),
    renderWiki,
    runWikiItem,
    navNext: () => {
      const q = wikiQuery.toLowerCase()
      ...
      wikiIdx = (wikiIdx + 1) % filtered.length
      renderWiki()
    },
    navPrev: () => {
      const q = wikiQuery.toLowerCase()
      ...
      if (!filtered.length) return
      wikiIdx = (wikiIdx - 1 + filtered.length) % filtered.length
      renderWiki()
    },
  }
}
...
export function getWikiState() {
  return { wikiActive, wikiMenuEl, wikiQuery, wikiItems, wikiIdx }
}

export function wikiNavNext() {
  if (!wikiItems.length) return
  const q = wikiQuery.toLowerCase()
  const filtered = wikiItems.filter(i => i.title.toLowerCase().includes(q) || (i.snippet && i.snippet.toLowerCase().includes(q)))
  if (!filtered.length) return
  wikiIdx = (wikiIdx + 1) % filtered.length
  renderWiki()
}

export function wikiNavPrev() {
  if (!wikiItems.length) return
  const q = wikiQuery.toLowerCase()
  const filtered = wikiItems.filter(i => i.title.toLowerCase().includes(q) || (i.snippet && i.snippet.toLowerCase().includes(q)))
  if (!filtered.length) return
...
export { checkWiki, closeWiki, renderWiki, runWikiItem }